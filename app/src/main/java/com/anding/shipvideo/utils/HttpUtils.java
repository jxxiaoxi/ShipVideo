...
public class HttpUtils {
     ...
     * @return
     */
    public void getDataAsyn(String url, final NetWorkCallBack netWorkCallBack) {
        //1 构造Request
        Request.Builder builder = new Request.Builder();
        Request request = builder.get().url(url).build();
        //2 将Request封装为Call
     ...
     * @param netWorkCallBack
     */
    public void postDataAsyn(String url, Map<String, String> bodyParams, final NetWorkCallBack netWorkCallBack) {
        //1构造RequestBody
        RequestBody body = setRequestBody(bodyParams);
        //2 构造Request
        Request.Builder requestBuilder = new Request.Builder();
    ...
    public void postJsonAsyn(String url, String json, final NetWorkCallBack myNetCall) throws IOException {
        RequestBody body = RequestBody.create(JSON, json);
        //2 构造Request
        Request.Builder requestBuilder = new Request.Builder();
        Request request = requestBuilder.post(body).url(url).build();
        ...
        okHttpClient.newCall(request).enqueue(new Callback() {
            ...
            public void onResponse(Call call, Response response) throws IOException {
                ...
                try {
                    ...
                    e.printStackTrace();
                    downloadListener.loadfail("loadfail");
                } finally {
                    try {
                        if (is != null)
                            is.close();
